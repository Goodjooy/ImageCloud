...
* ```/file```
  ...
  * 參數列表
      | 方法 | 参数列表 | 参数描述 | 功能 | 用户限制 | 响应模型 |
      | :--: | :--: | :--: | :--: | :--: |:------: |
      | GET | path={*targetPath*}&<br>withHidden={*withHiddenItems*:**false**} | path->要獲取的文件夾或者文件信息的路徑;<br>withHidden(boolean)->是否忽略隐藏文件 | 獲取指定文件夾或文件信息<br>*路徑以```/root```開頭* | USER | ```Result<Item>``` |
